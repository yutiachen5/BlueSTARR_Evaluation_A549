...
def BuildModel(seqlen):
    ...
    for i in range(config.NumConv):
        ...
        x=Activation('relu')(x)
        # maxpooling layer before the last convolution layer
        # if i == config.NumConv - 2:
    ...
    # remove the very first positional encoding layer
    # if(config.NumAttn>0):
    #     x=x+keras_nlp.layers.SinePositionEncoding()(x)
    for i in range(config.NumAttn):
        skip=x
        # x=LayerNormalization()(x)
        # postional encoding layer before the first attention layer
        if i == 0:
            x=x+keras_nlp.layers.SinePositionEncoding()(x)
        x = TransformerEncoder(intermediate_dim=config.AttnKeyDim[i], # conv, attention
